...
if [ "$USE_KIND" = "true" ]; then
  ...
  if ! kubectl config current-context >/dev/null 2>&1; then
    ...
    KIND_CLUSTER=inference-e2e BBR_IMAGE_TAG=${BBR_E2E_IMAGE} make bbr-image-kind
  else
    ...
    KIND_CLUSTER=${current_kind_cluster} BBR_IMAGE_TAG=${BBR_E2E_IMAGE} make bbr-image-kind
  fi
else
  if ! kubectl config current-context >/dev/null 2>&1; then
...
echo "Found an active cluster. Running Go BBR e2e tests in ./test/e2e/bbr/..."
go test ./test/e2e/bbr/ -v -ginkgo.v
