...
A chart to deploy an EndpointPicker (epp) deployment.  

## Install

To install an EndpointPicker named `vllm-llama3-8b-instruct`  that selects from endpoints with label `app: vllm-llama3-8b-instruct` and listening on port `8000`, you can run the following command:

```txt
$ helm install vllm-llama3-8b-instruct ./config/charts/inferencepool/charts/epp \
...
$ helm install vllm-llama3-8b-instruct \
  ...
  oci://us-central1-docker.pkg.dev/k8s-staging-images/gateway-api-inference-extension/charts/inferencepool/charts/epp --version v0
```

Note that the provider name is needed to deploy provider-specific resources. If no provider is specified, then only the EPP are deployed.
...
```txt
$ helm install vllm-llama3-8b-instruct ./config/charts/inferencepool/charts/epp -f values.yaml
```
...
$ helm install triton-llama3-8b-instruct \
  --set inferenceExtension.endpointsServer.endpointSelector="app=triton-llama3-8b-instruct" \
  --set inferenceExtension.endpointsServer.modelServerType=triton-tensorrt-llm \
  --set global.provider.name=[none|gke|istio] \
  oci://us-central1-docker.pkg.dev/k8s-staging-images/gateway-api-inference-extension/charts/inferencepool/charts/epp --version v0
```
...
* Via `--set` flag:
  ...
  ```txt
  helm install vllm-llama3-8b-instruct \
  --set inferenceExtension.endpointsServer.endpointSelector==vllm-llama3-8b-instruct \
  --set inferenceExtension.replicas=3 \
  --set global.provider=[none|gke] \
  oci://us-central1-docker.pkg.dev/k8s-staging-images/gateway-api-inference-extension/charts/inferencepool/charts/epp --version v0
  ```
...
| `inferenceExtension.endpointsServer.standalone`           | Whether enables the standalone mode or not. Defaults to`true`.
| `inferenceExtension.endpointsServer.modelServerType`      | Type of the model servers in the endpoint picker, valid options are [vllm, triton-tensorrt-llm], default is vllm. 
| `inferenceExtension.endpointsServer.targetPorts`          | Target port number for the vllm backends, will be used to scrape metrics by the inference extension. Defaults to 8000. 
| `inferenceExtension.endpointsServer.endpointSelector`     | Label selector to match vllm backends managed by the endpoint picker. "Format: a comma-separated list of key=value pairs without whitespace (e.g., 'app=vllm-llama3-8b-instruct,env=prod').")
...
| `global.provider.gke.autopilot`                           | Set to `true` if the cluster is a GKE Autopilot cluster. This is only used if `global.provider.name` is `gke`. Defaults to `false`.                                                                                                                |
