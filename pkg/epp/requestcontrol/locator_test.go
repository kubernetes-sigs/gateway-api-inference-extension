...
func TestDatastoreEndpointCandidates_Locate(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.ElementsMatch(t, tc.expectedPodIPs, gotIPs, "Locate returned unexpected set of endpoint candidates")
		})
	}
}
...
func (m *mockEndpointCandidates) Locate(ctx context.Context, _ map[string]any) []backendmetrics.PodMetrics {
	m.mu.Lock()
	m.calls++
	delay := m.delay
	result := m.result
