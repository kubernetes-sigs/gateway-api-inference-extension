...
const DefaultMaxNumOfEndpoints = 1 // common default to all pickers

// Parameters defines the common parameters for all pickers.
type Parameters struct {
	MaxNumOfEndpoints int `json:"maxNumOfEndpoints"`
}

// NormalizeMaxNumOfEndpoints applies the shared default when the provided value is invalid.
func NormalizeMaxNumOfEndpoints(maxNumOfEndpoints int) int {
	if maxNumOfEndpoints <= 0 {
		return DefaultMaxNumOfEndpoints
	}
	return maxNumOfEndpoints
}

// ShuffleScoredPods randomizes the order of scored pods in place.
