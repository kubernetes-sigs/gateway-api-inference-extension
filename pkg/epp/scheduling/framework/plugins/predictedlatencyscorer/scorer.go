...
package predictedlatencyscorer
...
import (
	...
	prefixcache "sigs.k8s.io/gateway-api-inference-extension/pkg/epp/scheduling/framework/plugins/prefixcachescorer"
...
func (s *SLOAwareRouter) getPrefixCacheScoreForPod(ctx context.Context, cycleState *schedulingtypes.CycleState, pod schedulingtypes.Pod) float64 {
	...
	plugintype := prefixcache.PrefixCachePluginType
	pluginname := prefixcache.PrefixCachePluginType
	...
	prefixCacheState, ok := stateData.(*prefixcache.SchedulingContextState)
	if !ok {
	...
	matchLen := prefixCacheState.PrefixCacheServers[prefixcache.ServerID(pod.GetPod().NamespacedName)]
