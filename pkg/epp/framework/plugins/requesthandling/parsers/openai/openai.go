...
func (p *OpenAIParser) ParseRequest(ctx context.Context, body []byte, headers map[string]string) (*fwkrh.RequestBody, error) {
...
// extractRequestBody extracts the RequestBody from the given request body map using path-based detection.
func extractRequestBody(rawBody []byte, headers map[string]string) (*fwkrh.RequestBody, error) {
	...
	case conversationsAPI:
		...
		if err := json.Unmarshal(rawBody, &conversations); err == nil && len(conversations.Items) > 0 {
			return &fwkrh.RequestBody{Conversations: &conversations}, nil
		}
	...
	case responsesAPI:
		...
		if err := json.Unmarshal(rawBody, &responses); err == nil && responses.Input != nil {
			return &fwkrh.RequestBody{Responses: &responses}, nil
		}
	...
	case chatCompletionsAPI:
		...
		if err := json.Unmarshal(rawBody, &chatCompletions); err == nil {
			if err = validateChatCompletionsMessages(chatCompletions.Messages); err == nil {
				return &fwkrh.RequestBody{ChatCompletions: &chatCompletions}, nil
			}
		}
	...
	case completionsAPI:
		...
		if err := json.Unmarshal(rawBody, &completions); err == nil && completions.Prompt != "" {
			return &fwkrh.RequestBody{Completions: &completions}, nil
		}
