...
func (t *PredictedLatency) PreRequest(ctx context.Context, request *schedulingtypes.InferenceRequest, schedulingResult *schedulingtypes.SchedulingResult) {
	...
	id := request.LLM.Headers[reqcommon.RequestIdHeaderKey]
	val, ok := t.runningRequestLists.Load(endpointName)
	var endpointRequestList *requestPriorityQueue
	if !ok {
		endpointRequestList = newRequestPriorityQueue()
		t.runningRequestLists.Store(endpointName, endpointRequestList)
	} else {
		endpointRequestList = val.(*requestPriorityQueue)
	}
...
func (t *PredictedLatency) AdmitRequest(ctx context.Context, request *schedulingtypes.InferenceRequest, endpoints []schedulingtypes.Endpoint) error {
	...
	if !predictedLatencyCtx.hasValidEndpoint && request.LLM.Objectives.Priority < 0 {
		logger.V(logutil.DEBUG).Info("PredictedLatency.AdmitRequest: Rejecting a sheddable request as no valid endpoint available due to slo violation", "requestID", request.LLM.Headers[reqcommon.RequestIdHeaderKey])
