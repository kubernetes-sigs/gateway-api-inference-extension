...
type LatencyScorerConfig struct {
	...
	// positive-tier endpoints (those meeting SLO). Headroom = SLO - predicted.
	// Range: [0, inf). Higher values make the scorer favor endpoints based on
	// TTFT headroom over TPOT headroom. When no SLO is set (headroom always
	// negative), all endpoints go to the negative tier and this weight is not
	// used. Default: 0.8.
	TTFTWeight float64 `json:"ttftWeight,omitempty"`
	// TPOTWeight controls the relative importance of TPOT headroom when scoring
	// positive-tier endpoints. Range: [0, inf). Higher values make the scorer
	// favor endpoints with more TPOT headroom. Set to 0 for non-streaming
	// workloads. Default: 0.2.
	TPOTWeight float64 `json:"tpotWeight,omitempty"`

	// Negative headroom means the predicted latency exceeds the target SLO
	// (headroom = SLO - predicted < 0). When all endpoints are in the negative
	// tier, the scorer ranks them by deficit magnitude: the endpoint with the
	// smallest deficit (closest to meeting SLO) gets the highest score.
	// These weights control how TTFT and TPOT deficits are blended into a
	// single score. See README.md "Scoring Within a Tier" for the full algorithm.
	//
	// NegHeadroomTTFTWeight controls the relative importance of TTFT deficit
	// when scoring negative-tier endpoints. Range: [0, inf). Higher values
	// make the scorer favor endpoints with lower predicted TTFT (faster
	// first token / e2e latency). Default: 0.8.
	NegHeadroomTTFTWeight float64 `json:"negHeadroomTTFTWeight,omitempty"`
	// NegHeadroomTPOTWeight controls the relative importance of TPOT deficit
	// when scoring negative-tier endpoints. Range: [0, inf). Higher values
	// make the scorer favor endpoints with lower predicted TPOT (faster
	// token generation). Set to 0 for non-streaming workloads. Default: 0.2.
	NegHeadroomTPOTWeight float64 `json:"negHeadroomTPOTWeight,omitempty"`

	// EpsilonExploreNeg is the probability of scoring the negative tier when both
	// tiers have endpoints. When triggered, only negative-tier endpoints get
	// non-zero scores; positive-tier endpoints get score=0 for this request.
	// This ensures overloaded endpoints get occasional traffic for recovery.
	// Range: [0, 1]. Default: 0.01 (1%).
	EpsilonExploreNeg float64 `json:"epsilonExploreNeg,omitempty"`

	// "Sticky" endpoints are endpoints that have a high prefix cache score for
	// the current request, meaning the request's prompt is already cached on
	// that endpoint. Routing to a sticky endpoint avoids redundant prefill
	// computation, reducing TTFT. The affinity gate narrows the candidate set
	// to sticky endpoints so requests tend to be routed to endpoints that
	// already have their prefix cached.
	//
	// AffinityGateTauGlobal is the global prefix cache score threshold.
	// Endpoints with prefix cache score >= tau are considered sticky.
	// Applied to ALL candidates BEFORE tier classification.
	// This lives in the scorer (not a picker) because gating must happen before
	// normalization; normalizing across the gated subset produces different
	// relative scores than normalizing across all endpoints then picking.
	// Set to 0 to disable. Default: 0.99.
	AffinityGateTauGlobal float64 `json:"affinityGateTauGlobal,omitempty"`

	// AffinityGateTau is the within-tier prefix cache score threshold.
	// After the global gate and tier split, this further narrows to sticky
	// endpoints within each tier. Applied BEFORE normalization so that
	// headroom scores are computed relative to the sticky subset only.
	// Set to 0 to disable. Default: 0.80.
...
// LatencyScorerFactory creates a new Plugin plugin.
func LatencyScorerFactory(name string, rawParameters json.RawMessage, _ fwkplugin.Handle) (fwkplugin.Plugin, error) {
	...
	if len(rawParameters) > 0 {
		if err := json.Unmarshal(rawParameters, &config); err != nil {
			return nil, fmt.Errorf("failed to unmarshal config for Plugin: %w", err)
		}
	}
	return NewLatencyScorer(config).WithName(name), nil
}

func NewLatencyScorer(config LatencyScorerConfig) *Plugin {
	return &Plugin{
...
func (s *Plugin) WithName(name string) *Plugin {
...
// ttftHeadroom/tpotHeadroom are extracted from info for convenience in scoring math.
type epData struct {
...
func (s *Plugin) Score(ctx context.Context, _ *framework.CycleState, _ *framework.LLMRequest, endpoints []framework.Endpoint) map[framework.Endpoint]float64 {
	...
	if !hasPredictions {
		logger.V(logutil.DEBUG).Info("Plugin: no predictions, using composite fallback")
	...
	case len(positive) > 0 && len(negative) > 0:
		if rng.Float64() < s.config.EpsilonExploreNeg {
			logger.V(logutil.DEBUG).Info("Plugin: scoring negative tier (epsilon explore)")
			s.scoreNegativeTier(ctx, negative, scores, rng)
		} else {
			logger.V(logutil.DEBUG).Info("Plugin: scoring positive tier")
...
// This acts as a secondary, independent gate. If the global gate (0.99) found no matches
// and fell back to all endpoints, this provides a "second chance" to find "good enough"
// (e.g., 0.80) cache matches. Applied BEFORE normalization.
func (s *Plugin) applyWithinTierAffinityGate(ctx context.Context, data []epData, rng *rand.Rand, tierLabel string, tau ...float64) []epData {
	...
	if rng.Float64() < s.config.EpsilonExploreSticky {
		logger.V(logutil.DEBUG).Info("Plugin: within-tier affinity gate exploring",
	...
	if s.config.AffinityMaxTTFTPenaltyMs > 0 {
		...
		if bestAll < math.MaxFloat64 && bestSticky < math.MaxFloat64 {
			...
			if penalty > s.config.AffinityMaxTTFTPenaltyMs {
				logger.V(logutil.DEBUG).Info("Plugin: within-tier TTFT penalty too high",
	...
	logger.V(logutil.DEBUG).Info("Plugin: within-tier affinity gate applied",
		"tier", tierLabel, "sticky", len(sticky), "total", len(data))
	return sticky
}

// scorePositiveTier scores positive headroom endpoints.
func (s *Plugin) scorePositiveTier(ctx context.Context, data []epData, scores map[framework.Endpoint]float64, rng *rand.Rand) {
	...
	for _, d := range data {
		...
		logger.V(logutil.TRACE).Info("Plugin: positive",
...
func (s *Plugin) scoreNegativeTier(ctx context.Context, data []epData, scores map[framework.Endpoint]float64, rng *rand.Rand) {
	logger := log.FromContext(ctx)
	...
	var selected []epData
	if len(idle) > 0 {
		logger.V(logutil.DEBUG).Info("Plugin: preferring idle pods in negative tier", "idle", len(idle))
		selected = idle
	} else {
		selected = busy
	}
	...
	// Step 2: Hierarchical deficit buckets. Endpoints are grouped by which SLOs they violate.
	// Higher priority (scored higher) is given to buckets with less severe violations.
	// Priority:
	// 1. nonNegTTFTnegTPOT (Only TPOT violates) - Preferred because TTFT impacts perceived responsiveness most.
	// 2. negTTFTnonNegTPOT (Only TTFT violates)
	// 3. negTTFTnegTPOT (Both violate - Worst)
...
func scoreNegativeBucketPerBucket(ctx context.Context, bucket []epData, scores map[framework.Endpoint]float64, alpha, beta float64, category string) {
	...
	logger.V(logutil.DEBUG).Info("Plugin: negative bucket normalization",
		"category", category,
	...
	for _, dd := range defs {
		...
		logger.V(logutil.TRACE).Info("Plugin: negative deficit",
...
// or timed out). It uses the same signals as the independent kv/queue/prefix
// scorers but applies them within this scorer's affinity-gated and tier-selected
// subset. This fallback exists because the framework doesn't support conditional
// scorer execution ("run A; if no results, run B instead").
// Also used by the "composite-only" strategy for SLO-aware composite routing.
func (s *Plugin) compositeScores(ctx context.Context, endpoints []framework.Endpoint) map[framework.Endpoint]float64 {
	...
	for _, ep := range endpoints {
		...
		scores[ep] = score
		logger.V(logutil.TRACE).Info("Plugin: composite",
