...
func newMockPodMetrics(name string, metrics *backendmetrics.Metrics) *backendmetrics.FakePodMetrics {
...
func TestDetector_IsSaturated(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			config: defaultConfig,
			pods: []*backendmetrics.FakePodMetrics{
				newMockPodMetrics("pod1", &backendmetrics.Metrics{
					UpdateTime:          baseTime,
					WaitingQueueSize:    2,
		...
		},
		{
			...
			pods: []*backendmetrics.FakePodMetrics{
				...
				newMockPodMetrics("pod2", &backendmetrics.Metrics{
		...
		},
		{
			...
			pods: []*backendmetrics.FakePodMetrics{
				...
				newMockPodMetrics("pod3", &backendmetrics.Metrics{
					UpdateTime:          baseTime,
					WaitingQueueSize:    1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := detector.IsSaturated(); got != tt.expectedSaturat {
