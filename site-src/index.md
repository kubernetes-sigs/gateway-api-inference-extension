...
- **Body Based Router(BBR)**: An additional (optional) implementation of an `Inference Scheduler` that uses information from the body portion of the inference request (currently the model name attribute from the body of an OpenAI API request)  to enable model-aware functions such as routing/scheduling. This may be used along with the EPP in order to have a combination of model picking and endpoint picking functionality.
